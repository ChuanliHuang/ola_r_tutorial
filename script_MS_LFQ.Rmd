...
data_withB3 <- read.table("AP_Expt3_proteinGroups_all.txt", header = TRUE, sep = "\t") 
data_withB3 <- data_withB3[data_withB3$Reverse != "+" & data_withB3$Potential.contaminant != "+" 
             &data_withB3$Only.identified.by.site !="+",]
data_withB3$Protein.IDs <- gsub("_SK1", "", data_withB3$Protein.IDs)

data_withB3[,allORFs.idcols] <- allORFs[match(data_withB3$Protein.IDs, allORFs$input),allORFs.idcols]
names(data_withB3)
```
...
```{r}
new_nameB3<-"Gene.names"
colnames_p_B3<-names(data_withB3)
colnames_p_B3[171]<-new_nameB3 #this has to be done manually. For every dataset this number is different. You want to replace the 'symbol' column name. In my case this is column 155
names(data_withB3)<-colnames_p_B3
```
Remove duplicated columns.

```{r}
#are there any duplicated genes?
data_withB3$Gene.names %>% duplicated() %>% any()
#how many?
data_withB3 %>% group_by(Gene.names) %>% summarize(frequency = n()) %>% 
  arrange(desc(frequency)) %>% filter(frequency > 1)
#make unique
data_unique_B3 <- make_unique(data_withB3, "Gene.names", "Protein.IDs", delim = ";")
data_withB3$name %>% duplicated() %>% any()
data_unique_B3$name<- make.names(data_unique_B3$name, unique=TRUE)
```

Add experimental design (a file explaining which sample belongs to which condition etc)

```{r}
exp_design_withB3 <- read.csv("Dsn1_ExpDesign.csv")
exp_design_withB3$label<-as.character(exp_design_withB3$label)
exp_design_withB3$condition<-as.character(exp_design_withB3$condition)
exp_design_withB3$replicate<-as.numeric(exp_design_withB3$replicate)
```
now generate unique identifiers
```{r}
LFQ_columns_withB3 <- grep("LFQ.", colnames(data_unique_B3))
data_se_withB3 <- make_se(data_unique_B3, LFQ_columns_withB3, exp_design_withB3) #this doesn't work--works if expdesign correct
data_se_parsed_withB3 <- make_se_parse(data_unique_B3, LFQ_columns_withB3) #this works even when expdesign not correct
```
...
save(allORFs, data, data_diff, data_diff_control, data_filt, data_filt_B3, data_filt_parsed_B3, data_se, data_norm, data_imp, data_se_parsed, data_se_parsed_withB3, data_se_withB3, data_unique, data_unique_B3, data_withB3, dep, dep_control,  dep_relaxed, DETdata, df_wide, exp_design, matrix, 
